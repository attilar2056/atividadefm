(function (global) {
  ...
  class UniversalRadioMetadata {
     ...
     */
    _loadSchedule(forceFresh) {
      const now = Date.now();
      const refreshMs = 4 * 60 * 1000;
      const shouldReuseCache = !forceFresh
        && this.state.programSchedule
        && this.state.programSchedule.length
        && this.state.scheduleCachedAt
        && (now - this.state.scheduleCachedAt) < refreshMs;

      if (shouldReuseCache) {
        return Promise.resolve(this.state.programSchedule);
      }
      if (!forceFresh && this.state.scheduleFetchPromise) {
        return this.state.scheduleFetchPromise;
      }
      const src = this.options.programsSrc || 'programas/programacao.json';
      const url = src;
      this.state.scheduleFetchPromise = fetch(url, { cache: 'no-store' })
        .then((res) => {
          if (!res.ok) {
        ...
        .then((data) => {
          ...
          this.state.scheduleCachedAt = Date.now();
          return normalized;
        })
        .catch((err) => {
          this.state.programSchedule = null;
          this.state.scheduleCachedAt = 0;
          throw err;
        })
        .finally(() => {
    ...
    _getClockContextFallback() {
      const timeZone = this.options.timeZone || 'America/Sao_Paulo';
      ...
      const weekdayMap = { sun: 'dom', mon: 'seg', tue: 'ter', wed: 'quar', thu: 'qui', fri: 'sex', sat: 'sab' };
      const weekday = weekdayMap[String(map.weekday || '').toLowerCase()] || 'seg';
    ...
    _getProgramMatchScore(item, clock) {
      const startMinutes = this._timeToMinutes(item.start);
      const endMinutes = this._timeToMinutes(item.end);
      if (startMinutes === null || endMinutes === null || !clock) return null;
      const dias = Array.isArray(item.dias) ? item.dias : [];
      const hasDayFilter = dias.length > 0;
      const previousWeekdayMap = { dom: 'sab', seg: 'dom', ter: 'seg', quar: 'ter', qui: 'quar', sex: 'qui', sab: 'sex' };

      if (endMinutes >= startMinutes) {
        if (hasDayFilter && dias.indexOf(clock.weekday) === -1) return null;
        if (clock.minutes < startMinutes || clock.minutes > endMinutes) return null;
        return startMinutes;
      }

      if (clock.minutes >= startMinutes) {
        if (hasDayFilter && dias.indexOf(clock.weekday) === -1) return null;
        return startMinutes;
      }

      if (clock.minutes <= endMinutes) {
        if (hasDayFilter && dias.indexOf(previousWeekdayMap[clock.weekday] || 'dom') === -1) return null;
    ...
    _findCurrentProgramInSchedule(schedule, clock) {
      let current = null;
      let currentScore = -Infinity;
      schedule.forEach((item) => {
        const score = this._getProgramMatchScore(item, clock);
    ...
    async _getCurrentProgram() {
      try {
        const runtime = typeof window !== 'undefined' ? window.__reScheduleRuntime : null;
        if (runtime && typeof runtime.getCurrentProgramByConfig === 'function') {
          const currentFromRuntime = await runtime.getCurrentProgramByConfig({
            programsSrc: this.options.programsSrc || '',
            timeApiUrl: this.options.timeApiUrl || '',
            timeZone: this.options.timeZone || ''
          }).catch(() => null);
          if (currentFromRuntime) {
            return {
              title: String(currentFromRuntime.title || currentFromRuntime.programa || '').trim(),
              host: String(currentFromRuntime.host || currentFromRuntime.locutor || '').trim()
            };
          }
        }
      } catch (_error) {}

      const schedule = await this._loadSchedule(false).catch(() => null);
      if (!schedule || !Array.isArray(schedule) || schedule.length === 0) return null;
      const clock = this._getClockContextFallback();
      return this._findCurrentProgramInSchedule(schedule, clock);
    }
