  ...
  class UniversalRadioMetadata {
    constructor(options) {
      this.options = deepMerge(DEFAULTS, options || {});
         ...
         */
        pendingTimer: null,
        forcedStateTimer: null,
        isForcedProgramActive: false,
        suppressedRawTitle: ''
      };
    ...
    start() {
      this._ensureMounted();
      this.reset();
      this._startForcedStateLoop();
      this._syncForcedState();
    ...
    stop() {
      ...
      if (this.state.forcedStateTimer) {
        clearInterval(this.state.forcedStateTimer);
        this.state.forcedStateTimer = null;
      }
    ...
    reset() {
      ...
      const forced = this._getForcedCard();
      if (forced) {
        this.state.isForcedProgramActive = true;
        this._render(forced);
        return this;
      }
      const overrideState = this._getMetadataOverrideState();
      const placeholder = overrideState && overrideState.placeholder ? overrideState.placeholder : null;
      this.state.isForcedProgramActive = false;
      this._render({
        song: placeholder && placeholder.song ? placeholder.song : (this.options.stationName || this.options.textFallback || 'Ao vivo'),
        artist: '',
        cover: placeholder && placeholder.cover ? placeholder.cover : (this.options.defaultCover || ''),
        rawTitle: ''
      });
      return this;
    }
    ...
    async pushRawTitle(rawTitle) {
      ...
      const forced = this._getForcedCard();
      if (forced) {
        this.state.suppressedRawTitle = raw;
        this.state.isForcedProgramActive = true;
        this._render(forced);
        return forced;
      }

      if (raw === this.state.lastRawTitle) return null;
    ...
    _getMetadataOverrideState() {
      try {
        if (typeof window !== 'undefined' && typeof window.__reGetMetadataOverrideState === 'function') {
          const state = window.__reGetMetadataOverrideState();
          const placeholder = state && state.placeholder ? {
            song: String(state.placeholder.title || this.options.stationName || this.options.textFallback || 'Ao vivo'),
            artist: '',
            cover: String(state.placeholder.cover || this.options.defaultCover || '').trim(),
            rawTitle: ''
          } : null;
          const forced = state && state.forced ? {
            song: String(state.forced.title || this.options.stationName || this.options.textFallback || 'Ao vivo'),
            artist: '',
            cover: String(state.forced.cover || this.options.defaultCover || '').trim(),
            rawTitle: ''
          } : null;
          return {
            mode: state && state.mode ? String(state.mode) : 'normal',
            placeholder,
            forced
          };
        }
      } catch (_error) {}
      return { mode: 'normal', placeholder: null, forced: null };
    }

    _getForcedCard() {
      const overrideState = this._getMetadataOverrideState();
      if (overrideState && overrideState.forced) {
        return overrideState.forced;
      }
      try {
        if (typeof window !== 'undefined' && typeof window.__reGetForcedMetadataCard === 'function') {
          const card = window.__reGetForcedMetadataCard();
          if (card && card.active) {
            return {
              song: String(card.title || this.options.stationName || this.options.textFallback || 'Ao vivo'),
              artist: '',
              cover: String(card.cover || this.options.defaultCover || '').trim(),
              rawTitle: ''
            };
          }
        }
    ...
    async _renderRawTitleImmediately(rawTitle) {
      const raw = String(rawTitle || '').trim();
      if (!raw) {
        this.reset();
        return null;
      ...
      const immediate = this.parseRawTitle(raw);
      const resolved = await this.resolveTrackMetadata(raw).catch(() => immediate);
      this.state.lastRawTitle = raw;
      this._render({
        song: resolved.song || immediate.song || raw,
        artist: resolved.artist || immediate.artist || '',
    ...
    _startForcedStateLoop() {
      if (this.state.forcedStateTimer) return;
      const intervalMs = Math.max(500, Number(this.options.forceStatePollMs || 1000) || 1000);
      this.state.forcedStateTimer = setInterval(() => {
        // Não sincronizar estado forçado quando a aba está em segundo plano —
        // evita trabalho desnecessário que pode causar engasgos no stream.
        // O estado será sincronizado imediatamente ao retornar ao primeiro plano.
        if (typeof document !== 'undefined' && document.hidden) return;
        this._syncForcedState();
      }, intervalMs);

      // Ao retornar ao primeiro plano, sincronizar o estado imediatamente
      // para que os metadados e banners sejam atualizados sem atraso.
      // Também reconecta ao SSE para capturar qualquer mudança que ocorreu
      // enquanto a aba estava oculta.
      if (!this._boundVisibilityChange) {
        this._boundVisibilityChange = () => {
          if (!document.hidden) {
            // Pequeno atraso para deixar o stream estabilizar antes de atualizar UI
            setTimeout(() => {
              // Sincronizar estado forçado (programas/banners)
              this._syncForcedState();
              
              // Reconectar ao SSE para capturar metadados perdidos
              // Se a música mudou enquanto estávamos em background, o SSE
              // pode ter perdido o evento. Reconectar garante que receberemos
              // o estado atual do stream.
              if (this.options.metadata && this.options.metadata.type === 'sse') {
                this._connectEventSource();
              }
            }, 80);
          }
    ...
    _syncForcedState() {
      ...
      if (this.state.isForcedProgramActive) {
        this.state.isForcedProgramActive = false;
        const suppressed = this.state.suppressedRawTitle;
        this.state.suppressedRawTitle = '';
        if (suppressed) {
          this._renderRawTitleImmediately(suppressed);
        } else {
          this.reset();
        }
      }
    ...
    async _showPlaceholder() {
      const overrideState = this._getMetadataOverrideState();
      if (overrideState && overrideState.placeholder) {
        this._render(overrideState.placeholder);
        return;
      }

      let title = '';
      let cover = this.options.defaultCover || '';
      try {
