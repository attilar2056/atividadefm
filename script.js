(function () {
  ...
  function createVirtualScheduleHost(config) {
    var safeConfig = config || {};
    return {
      getAttribute: function (name) {
        if (name === 'data-programs-src') return safeConfig.programsSrc || safeConfig.programSource || '';
        if (name === 'data-time-api-url') return safeConfig.timeApiUrl || safeConfig.clockApiUrl || '';
        if (name === 'data-timezone') return safeConfig.timeZone || safeConfig.timezone || '';
        return '';
      }
    };
  }

  function getRuntimeClockContextByConfig(config) {
    if (!config) return currentRuntimeClockContext();
    var virtualHost = createVirtualScheduleHost(config);
    primeClockForWidget(virtualHost);
    return currentClockContext(virtualHost);
  }
  ...
  function getProgramMatchScore(item, clock) {
    var start = timeToMinutes(item && item.start);
    var end = timeToMinutes(item && item.end);
    if (start === null || end === null || !clock) return null;
    var days = normalizeWeekdays(item.diaDaSemana || []);
    var hasDayFilter = days.length > 0;

    if (end >= start) {
      if (hasDayFilter && days.indexOf(clock.weekday) === -1) return null;
    ...
    if (clock.minutes <= end) {
      if (hasDayFilter && days.indexOf(previousWeekday(clock.weekday)) === -1) return null;
      return start - (24 * 60);
    }
  ...
  function findProgram(items, clock) {
    ...
    for (var i = 0; i < items.length; i += 1) {
      var score = getProgramMatchScore(items[i], clock);
  ...
  function getCurrentProgramByConfig(config) {
    var host = config ? createVirtualScheduleHost(config) : getClockWidgetForRuntime();
    return loadProgramsForWidget(host, { forceFresh: !!(config && config.forceFresh) }).then(function (items) {
      var clock = config ? getRuntimeClockContextByConfig(config) : currentRuntimeClockContext();
      return findProgram(items || [], clock) || null;
    });
  }

  if (typeof window !== 'undefined') {
    window.__reScheduleRuntime = {
      getClockContext: function (config) {
        return config ? getRuntimeClockContextByConfig(config) : currentRuntimeClockContext();
      },
      getCurrentProgramByConfig: getCurrentProgramByConfig,
      primeClockByConfig: function (config) {
        var host = config ? createVirtualScheduleHost(config) : getClockWidgetForRuntime();
        primeClockForWidget(host);
      }
    };
  }
    ...
    widgets.forEach(function (widget) {
      ...
      try {
        ...
        widget.__radioMetadataInstance = new window.UniversalRadioMetadata({
          ...
          programsSrc: getProgramsSourceUrl(widget),
          timeApiUrl: getTimeApiUrl(widget),
          timeZone: getTimezoneForWidget(widget),
          theme: {
            injectCss: true,
            borderRadius: '18px',
            gap: '16px'
