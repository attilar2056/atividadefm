(function () {
  ...
  var VOZ_STATE_REFRESH_MS = 5 * 1000;
  var RADIO_STREAM_TIMEOUT_MS = 12 * 1000;
  var RADIO_SILENCE_TIMEOUT_MS = 15 * 1000;
  var VOZ_PROGRAM_TITLE = 'A Voz do Brasil';
  var MUSIC_PROGRAM_TITLE = '1 Hora Só de Músicas';
  ...
  function getTimeApiUrl(widget) {
    var sourceHost = findScheduleConfigHost(widget);
    return (sourceHost && sourceHost.getAttribute('data-time-api-url')) || 'https://time.now/developer/api/timezone/America/Sao_Paulo';
  }
  ...
  function shouldUseDirectImageCache(src) {
    var safeSrc = String(src || '').trim();
    if (!safeSrc) return false;
    return /^https:\/\//i.test(safeSrc) && /\.gif(?:$|\?)/i.test(safeSrc);
  }

  function getFilenameFromPath(src) {
    var safeSrc = String(src || '').trim();
    if (!safeSrc) return '';
    var cleanSrc = safeSrc.split('#')[0].split('?')[0];
  ...
  function getVinylGifSource(staticSrc) {
    ...
    var filename = getFilenameFromPath(safeStaticSrc);
    if (filename && VINYL_GIF_HTTPS_MAP[filename]) return VINYL_GIF_HTTPS_MAP[filename];
  ...
  function primeVinylGifCache(src) {
    var safeSrc = String(src || '').trim();
    if (!safeSrc || typeof Image === 'undefined') return;
    if (!window.__reVinylGifCache) window.__reVinylGifCache = Object.create(null);
    if (window.__reVinylGifCache[safeSrc]) return;
    var img = new Image();
    img.decoding = 'async';
    img.loading = 'eager';
    img.referrerPolicy = 'no-referrer';
    img.src = safeSrc;
    window.__reVinylGifCache[safeSrc] = img;
  }
  ...
  function getVoiceStateStore() {
    if (typeof window === 'undefined') {
      return { active: false, fetchedAt: 0, promise: null, lastSource: '' };
    }
    if (!window.__reVoiceStateStore) {
      window.__reVoiceStateStore = {
        active: false,
        fetchedAt: 0,
        promise: null,
        lastSource: ''
    ...
    return window.__reVoiceStateStore;
  }
  ...
  function primeVoiceState() {
    if (typeof window !== 'undefined' && window.location && window.location.protocol === 'file:') {
      return Promise.resolve(false);
    }
    var store = getVoiceStateStore();
    var now = Date.now();
    if (store.promise) return store.promise;
    if (store.fetchedAt && (now - store.fetchedAt) < VOZ_STATE_REFRESH_MS) return Promise.resolve(!!store.active);

    store.promise = fetch(buildNoCacheUrl(VOZ_STATE_URL, now), {
      cache: 'no-store',
      headers: { 'Accept': 'application/json, text/plain, */*' }
    }).then(function (res) {
      if (!res.ok) throw new Error('Falha ao carregar voz.json');
      return res.text();
    }).then(function (text) {
      var parsed = text;
      try {
      ...
      store.active = !!readVoiceStateFromPayload(parsed);
      store.fetchedAt = Date.now();
      store.lastSource = VOZ_STATE_URL;
      store.promise = null;
      return store.active;
    }, function () {
      store.fetchedAt = Date.now();
  ...
  function shouldCheckVoiceState(clock) {
    ...
    return ctx.minutes >= ((19 * 60) - 1) && ctx.minutes <= ((19 * 60) + 59);
  }

  function isVoiceStateActive(clock) {
    var ctx = clock || currentRuntimeClockContext();
    if (!shouldCheckVoiceState(ctx)) return false;
    return !!getVoiceStateStore().active;
  }

  function getRuntimeScheduleItems() {
    var widget = getClockWidgetForRuntime();
    var url = getProgramsSourceUrl(widget);
    var cache = PROGRAMS_SOURCE_CACHE[url];
    if (cache && Array.isArray(cache.items) && cache.items.length) return cache.items;
    return parseScheduleItemsFromWidgetFallback(widget).map(cloneScheduleItem).filter(Boolean);
  }

  function findVoiceProgramItem(clock) {
    var ctx = clock || currentRuntimeClockContext();
    var items = getRuntimeScheduleItems();
    if (!items.length) return null;
    var current = findProgram(items, ctx);
    if (current && String(stripAccents(current.title || '')).toLowerCase().indexOf('voz do brasil') !== -1) {
      return current;
    }
    for (var i = 0; i < items.length; i += 1) {
      var item = items[i];
      if (String(stripAccents(item.title || '')).toLowerCase().indexOf('voz do brasil') !== -1) return item;
    }
    return null;
  }

  function findMusicProgramItem(clock) {
    var ctx = clock || currentRuntimeClockContext();
    var items = getRuntimeScheduleItems();
    if (!items.length) return null;
    var normalizedTitle = String(stripAccents(MUSIC_PROGRAM_TITLE || '')).toLowerCase();
    var weekday = ctx && ctx.weekday ? ctx.weekday : '';
    var exactWeekday = null;
    var laterWeekday = null;
    var anyMusic = null;
    for (var i = 0; i < items.length; i += 1) {
      var item = items[i];
      var itemTitle = String(stripAccents(item && item.title || '')).toLowerCase();
      if (itemTitle.indexOf(normalizedTitle) === -1) continue;
      if (!anyMusic) anyMusic = item;
      var itemDays = Array.isArray(item && item.days) ? item.days : [];
      if (weekday && itemDays.indexOf(weekday) === -1) continue;
      if (!exactWeekday) exactWeekday = item;
      var itemStart = Number(item && item.startMinutes);
      if (!laterWeekday && Number.isFinite(itemStart) && itemStart >= (20 * 60)) {
        laterWeekday = item;
      }
    }
    return laterWeekday || exactWeekday || anyMusic || null;
  }

  function resolveMetadataOverrideState(clock) {
    var ctx = clock || currentRuntimeClockContext();
    var state = {
      mode: 'normal',
      forced: null,
    ...
    if (!ctx || typeof ctx.minutes !== 'number') return state;
    if (!isVoiceOfBrazilWeekday(ctx)) return state;
    if (ctx.minutes < (19 * 60) || ctx.minutes > ((19 * 60) + 59)) return state;

    if (isVoiceStateActive(ctx)) {
      var voiceItem = findVoiceProgramItem(ctx);
      var voiceCard = {
        active: true,
        title: VOZ_PROGRAM_TITLE,
        artist: '',
        cover: (voiceItem && voiceItem.image) || '',
        rawTitle: ''
      };
      state.mode = 'voice';
      state.forced = voiceCard;
      state.placeholder = voiceCard;
      return state;
    }

    var musicItem = findMusicProgramItem(ctx);
    state.mode = 'music';
    state.placeholder = {
      active: false,
      title: MUSIC_PROGRAM_TITLE,
      artist: '',
      cover: (musicItem && musicItem.image) || '',
      rawTitle: ''
    };
    return state;
  }

  function updateGlobalForcedMetadataState(clock) {
    if (typeof window === 'undefined') return;
    var overrideState = resolveMetadataOverrideState(clock);
    window.__reForcedMetadataCard = overrideState.forced || null;
    window.__reMetadataOverrideState = overrideState;
  }

  function getForcedMetadataCard() {
    if (typeof window === 'undefined') return null;
    var card = window.__reForcedMetadataCard;
    return (card && card.active) ? card : null;
  }

  if (typeof window !== 'undefined') {
    window.__reGetForcedMetadataCard = getForcedMetadataCard;
    window.__reGetMetadataOverrideState = function () {
      updateGlobalForcedMetadataState();
      return window.__reMetadataOverrideState || { mode: 'normal', forced: null, placeholder: null };
    };
  }
  ...
  function isVoiceOfBrazilWindow(clock) {
    return shouldCheckVoiceState(clock);
  }

  function resolveManagedRadioUrl(clock, host) {
    var ctx = clock || currentRuntimeClockContext();
    primeVoiceState();
    updateGlobalForcedMetadataState(ctx);
    return isVoiceStateActive(ctx) ? VOZ_DO_BRASIL_STREAM_URL : getConfiguredDefaultRadioStreamUrl(host);
  }
  ...
  function readClockContextFromApiPayload(data) {
    var directIso = data && typeof data.datetime === 'string' ? data.datetime : '';
    if (directIso) return buildClockContextFromIso(directIso, 0);
  ...
  function startAudioDetection(engine) {
    ...
    engine.detectInterval = setInterval(function () {
      ...
      if (!engine.audioDetected) {
        if (engine.wantPlay && engine.detectStartedAt && (Date.now() - engine.detectStartedAt) >= RADIO_STREAM_TIMEOUT_MS) {
          clearAudioDetection(engine);
      ...
      if (isManagedRadioUrl(engine.lastUrl, engine.hostNode || document.body)) {
  ...
  function bindPlayers() {
    ...
    loadProgramsForWidget(getClockWidgetForRuntime(), { forceFresh: false }).catch(function () {});
    primeVoiceState();
    ...
    function engineFromControl(control) {
      ...
      if (ownUrl) {
        ...
        if (!engines.has(runtimeId)) {
          ...
          audio.addEventListener('error', function () { if (isManagedRadioUrl(engine.lastUrl)) { handleManagedStreamFailure(engine, 'error'); return; } syncAll(); });
        }
    ...
    function syncHost(host, engine) {
      ...
      if (host.classList.contains('re-type-vinyl')) {
        ...
        if (vinylImg) {
          ...
          // Lógica de pausa em segundo plano com delay de retomada
          var isHidden = typeof document !== 'undefined' && document.hidden;
          var resumeAt = (typeof window !== 'undefined' && window.__reVinylResumeAt) || 0;
          var isWaitingToResume = Date.now() < resumeAt;
          
          // O GIF só deve ser usado se:
          // 1. O player estiver ativo (liveActive)
          // 2. A página NÃO estiver oculta
          // 3. O tempo de delay após voltar ao primeiro plano já tiver passado
          var shouldUseGif = liveActive && !isHidden && !isWaitingToResume;
    ...
    var runtimeClock = currentRuntimeClockContext();
    primeVoiceState();
    updateGlobalForcedMetadataState(runtimeClock);
    engines.forEach(function (engine) {
        ...
        var desiredUrl = resolveManagedRadioUrl(runtimeClock, host || document.body);
    ...
    syncManagedStreams();
    // Usar intervalo adaptativo: 1s em primeiro plano, 5s em segundo plano.
    // Isso reduz drasticamente o trabalho de CPU quando o usuário está em
    // outra aba ou com o site minimizado, evitando engasgos no stream.
    var syncManagedStreamsTimer = setInterval(function () {
      // syncManagedStreams já tem verificação interna de document.hidden,
      // mas rodando menos vezes em background economizamos ainda mais.
      if (!document.hidden) {
        syncManagedStreams();
      }
    }, 1000);
  ...
  function bindNewsWidgets() {
    ...
    widgets.forEach(function (widget) {
      ...
      // Chave de cache baseada na URL da API
      var cacheKey = 're-news-cache-' + btoa(apiUrl).substring(0, 16);
      var CACHE_DURATION = 15 * 60 * 1000; // 15 minutos
      ...
      function startRotation(items) {
        ...
        function renderGroup() {
          // Pausar renderização se estiver em segundo plano ou no período de delay
          var isHidden = typeof document !== 'undefined' && document.hidden;
          var resumeAt = (typeof window !== 'undefined' && window.__reVinylResumeAt) || 0;
          if (isHidden || Date.now() < resumeAt) return;
        ...
        if (items.length > itemsPerView) {
          rotationTimer = setInterval(function () {
            // Não rotacionar em segundo plano ou no período de delay
            var isHidden = typeof document !== 'undefined' && document.hidden;
            var resumeAt = (typeof window !== 'undefined' && window.__reVinylResumeAt) || 0;
            if (isHidden || Date.now() < resumeAt) return;
    ...
    widgets.forEach(function (widget) {
      ...
      if (items.length > 1) {
        setInterval(function () {
          // Pausar troca de banner em segundo plano ou no período de delay
          var isHidden = typeof document !== 'undefined' && document.hidden;
          var resumeAt = (typeof window !== 'undefined' && window.__reVinylResumeAt) || 0;
          if (isHidden || Date.now() < resumeAt) return;
  ...
  function bindSchedules() {
    ...
    function renderVinylWidget(widget, items) {
      ...
      // the program is playing, use an explicit HTTPS GIF mapping when
      // available, falling back to the local file with the same base name.
    ...
    // Tick de agendamento: 1s em primeiro plano, pausado em segundo plano.
    // Ao retornar ao primeiro plano, dispara imediatamente para garantir
    // que o programa atual seja exibido sem atraso.
    setInterval(tick, 1000);

    document.addEventListener('visibilitychange', function () {
      if (document.hidden) {
        // Ao esconder, forçamos a atualização para o JPG estático imediatamente
        if (typeof syncManagedStreams === 'function') syncManagedStreams();
      } else {
        // Define o timestamp para retomar o disco após 2 segundos
        if (typeof window !== 'undefined') {
          window.__reVinylResumeAt = Date.now() + 2000;
        }

        // Retornou ao primeiro plano: atualizar programa e metadados imediatamente
        setTimeout(function () {
          allWidgets.forEach(function (widget) { primeClockForWidget(widget); });
          renderFromCache();
        }, 150);
        
        // Sincronizar o estado do disco após o delay de 2 segundos
        setTimeout(function () {
          if (typeof syncManagedStreams === 'function') syncManagedStreams();
        }, 2050);

        // Depois, fazer refresh da fonte para capturar mudanças que ocorreram
        setTimeout(function () {
          refreshFromSource();
        }, 800);
      }
    ...
    if (typeof document !== 'undefined') {
      document.addEventListener('visibilitychange', function () {
        ...
        if (document.hidden) {
          ...
          // Período de graça de 3 segundos após retornar ao primeiro plano.
          // Evita que timers acumulados durante o background disparem todos
          // de uma vez e causem engasgos no stream de áudio.
          if (typeof window !== 'undefined') window.__reVisibilityResumeAt = Date.now() + 3000;
          engines.forEach(function (engine) {
            if (engine.wantPlay && !engine.detectInterval) {
              if (startDetection) {
                // Atraso maior para deixar o stream de áudio estabilizar
                // antes de retomar a detecção de presença de áudio.
                setTimeout(function () {
          ...
          // Sincronizar streams gerenciados após o período de graça
          setTimeout(function () {
            if (typeof syncManagedStreams === 'function') syncManagedStreams();
          }, 3200);
        }
