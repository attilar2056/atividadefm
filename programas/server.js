...
function handleSaveItemPayload(payload) {
  ...
  const currentDaySegments = dayMaps[parsed.day].map(cloneSegment);

  if (parsed.originalStartMin !== null && parsed.originalEndMin !== null) {
    dayMaps[parsed.day] = applySegment(dayMaps[parsed.day], makeEmptySegment(parsed.day, parsed.originalStartMin, parsed.originalEndMin));
  }

  const adjustedEndMin = findBlockingEnd(currentDaySegments, parsed.startMin, parsed.endMin, parsed.originalStartMin, parsed.originalEndMin);
  if (adjustedEndMin < parsed.startMin) {
    throw new Error('Esse horário invade o próximo programa. Ajuste o horário final.');
  }

  dayMaps[parsed.day] = applySegment(dayMaps[parsed.day], {
    day: parsed.day,
...
function handleDeleteItemPayload(payload) {
  ...
  const current = readScheduleFile();
  const dayMaps = buildDayMaps(current);
  dayMaps[day] = applySegment(dayMaps[day], makeEmptySegment(day, startMin, endMin));
  const saved = saveSchedule(dayMapsToSchedule(dayMaps, current));
  return buildAdminState(saved);
}
